...
def test_check_file_size_min():
    class UploadFileModel1(BaseModel):
        file: t.Annotated[UploadFile, AfterValidator(check_file_size())]
    ...
    class UploadFileModel3(BaseModel):
        file: t.Annotated[UploadFile, AfterValidator(check_file_size(min='0 KiB', max='1 KiB'))]

    class UploadFileModel4(BaseModel):
        file: t.Annotated[UploadFile, AfterValidator(check_file_size(min='1 KiB', max='1 KiB'))]

    class UploadFileModel5(BaseModel):
        file: t.Annotated[
            UploadFile, AfterValidator(check_file_size(min_inclusive=False, max_inclusive=False))
        ]
    ...
    with pytest.raises(ValueError, match=r'Must be greater than or equal to 2 KiB'):
        ...
        class UploadFileModel6(BaseModel):
            file: t.Annotated[UploadFile, AfterValidator(check_file_size(min='2 KiB', max='3 KiB'))]
    ...
    with pytest.raises(ValueError, match=r'Must be greater than or equal to 2 KiB'):
        ...
        class UploadFileModel7(BaseModel):
            file: t.Annotated[UploadFile, AfterValidator(check_file_size(min='2 KiB'))]
